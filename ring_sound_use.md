...
`accel_x`、`accel_y`、`accel_z` 是三轴加速度计数据，表示戒指沿设备 X、Y、Z 三个坐标轴测得的加速度：

| 字段 | 含义 |
| --- | --- |
| `accel_x` | 设备 X 轴方向的加速度原始值 |
| `accel_y` | 设备 Y 轴方向的加速度原始值 |
| `accel_z` | 设备 Z 轴方向的加速度原始值 |

加速度描述速度随时间的变化，不是移动速度或空间坐标。加速度计同时会测到重力，因此戒指静止时三个轴不一定为 `0`；通常会有一个或多个轴包含重力在当前设备姿态上的分量。移动、敲击、挥动和改变姿态都会使这些数值变化。

`gyro_x`、`gyro_y`、`gyro_z` 是三轴陀螺仪数据，表示戒指绕设备 X、Y、Z 三个坐标轴旋转的角速度：

| 字段 | 含义 |
| --- | --- |
| `gyro_x` | 绕设备 X 轴旋转的角速度原始值 |
| `gyro_y` | 绕设备 Y 轴旋转的角速度原始值 |
| `gyro_z` | 绕设备 Z 轴旋转的角速度原始值 |

角速度描述单位时间内旋转得多快，不是已经旋转的角度。正负号表示相反旋转方向，但具体哪个物理方向为正，取决于传感器坐标系和芯片在戒指中的安装方向。需要得到旋转角度时，可以对角速度按时间积分，但会累积零偏和漂移，通常需要结合加速度计做姿态融合。

当前 `0x0605` 中的六轴字段是设备上报的有符号 `16` 位原始值，SDK 不会自动转换成 `g` 或 `°/s`：

```text
ACCEL = (accel_x, accel_y, accel_z)  # 原始 i16
GYRO  = (gyro_x, gyro_y, gyro_z)     # 原始 i16
```

`start_sensor_report()` 的 `0x0602` 响应会返回：

- `sample_rate_hz`：每秒采样次数。
- `accel_range_g`：加速度计配置量程。
- `gyro_range_dps`：陀螺仪配置量程。

只有在设备固件确认原始值按完整 `int16` 满量程线性映射时，才能使用下面的换算：

```text
accel_g = accel_raw / 32768 * accel_range_g
gyro_dps = gyro_raw / 32768 * gyro_range_dps
```

当前公开协议没有承诺该换算比例，因此业务代码默认应保存和展示原始值。需要显示物理单位时，应先核对当前 IMU 芯片型号、量程寄存器配置、灵敏度和固件是否做过额外缩放。

批量和时间字段的含义：

| 字段 | 含义 |
| --- | --- |
| `sequence_start` | 本批第一个采样点的连续序号 |
| `frame_count` | 本批包含的采样点数量 |
| `sequence_start + frame_count - 1` | 本批最后一个采样点序号 |
| `timestamp_ms` | 设备侧采样时间戳，单位毫秒；不是 PC 当前日期时间 |

例如 `sequence_start=3536`、`frame_count=26`，表示本批包含序号 `3536` 至 `3561` 的 26 个采样点。序号可用于发现丢帧或重复帧；下一批正常应从 `3562` 开始。
